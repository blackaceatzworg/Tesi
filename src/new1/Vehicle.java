...
//			System.out.println(this.getId()+" accelera"+this.getSpeedZone());
		double delta=0;
...
//		System.out.println(this.getCurrentSpeed()+" +"+delta+"="+(this.getCurrentSpeed()+delta));
		if(this.getCurrentSpeed()+delta>5){
...
//			System.out.println(this.getId()+" frena, speezZone="+this.getSpeedZone());
		double delta=0;
		if(this.getCurrentSpeed()>=0&&this.getCurrentSpeed()<1){
			delta=0.486;
		}if(this.getCurrentSpeed()>=1&&this.getCurrentSpeed()<2){
		...
		if(this.getCurrentSpeed()>=4&&this.getCurrentSpeed()<=5.5){
			delta=0.324;
		}
//		System.out.println(this.getCurrentSpeed()+" -"+delta+"="+(this.getCurrentSpeed()-delta));
			if(this.getCurrentSpeed()-delta<=0){
...
//					freeride=false;
					finalDecision=this.manageVehiclePresence(ac.getIndex());
					stop=true;
					break;
				}
...
//					freeride=false;
					finalDecision=this.managePedestrianPresence(ac.getIndex());
					stop=true;
					break;
				}
...
//						freeride=false;
						finalDecision=this.manageAnticipationPresence(ac.getIndex());
						stop=true;
						break;
					}	
				}
...
//		System.out.println(this.getId()+" free @ v:"+this.getCurrentSpeed());
		Random r=new Random();
		double speedChangeChance=r.nextDouble();
		if(speedChangeChance<=0.70){
			this.speedUp();
		}else
		{
			this.speedDown();
	...
	public int manageVehiclePresence(int cellIndex){
		 Random r=new Random();
...
//					 this.speedDown();
	...
	public int manageAnticipationPresence(int cellIndex){
		 int speedZone=this.getSpeedZone();
		 int decision=0;
		 switch(speedZone){
		 case 0:
...
//				 this.speedDown();
	...
	public int managePedestrianPresence(int cellIndex){
		Random r=new Random();
		 double choice=r.nextDouble();
		 int decision=0;
		//valore intero di velocitˆ
